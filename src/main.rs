...
fn main() {
    ...
    let matches = App::new("lpm")
        ...
        .subcommand(SubCommand::with_name("update")
                    .about("Update all packages to introduce new code from its registered local dependencies"))
    ...
    match matches.subcommand() {
        ("add", Some(add_matches)) => {
            ...
            let package_path = fs::canonicalize(PathBuf::from(path)).expect(format!("Invalid path: {}", path).as_str());
...
fn load_registry(state: &State) -> Registry {
    ...
    state.package_paths.iter().for_each(|path| registry.add(Package::new(PathBuf::from(path))));
    registry
}
